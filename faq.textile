---
layout: default
title: Frequenrly Asked Questions
---
...
* *A*: Yes and no !
  ...
  example a "one to many" relation :
...
* *Q: There's already a plethora of ORMs in Java, do we _really_ need one more ?*

* *A:* Can you spot the typo in this JPA query ?
<script type="syntaxhighlighter" class="brush: java"><![CDATA[  
Query query = 
 em.createQuery("SELECT p FROM Product p WHERE p.param2 < :threshold ORDER BY p.param1 ascending");
 query.setParameter("thresold", my_threshold);
...
<script type="syntaxhighlighter" class="brush: java"><![CDATA[  
  ...
  Iterator<Course> coursesOfProffessorX = getProfessor(x).getCourses();
  
  while(coursesOfProffessorX.hasNext()) {
    Course c = coursesOfProffessorX.next();
	Iterator<Student> studentsOfCourseC = c.getStudents()
	while(studentsOfCourseC.hasNext()) {
	  Student s = studentsOfCourseC.next();
	  System.out.println("proffesor X teaches to " + s.getFullName());
	}
  }
]]></script>
