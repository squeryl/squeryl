...
slighly higher higher with Squeryl, but you get to reuse the code
...
or define a separate query for filtering, loosing the oportunity for reuse.

Since the *Artists.songs* method is a lazily evaluated Qurey[Song], it can
be further filtered with the 'where' method, yielding a unevaluated
Query[Song] wich will generate the SQL with the appropriate where clause.
