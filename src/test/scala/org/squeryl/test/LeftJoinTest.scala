...
abstract class LeftJoinTest extends SchemaTester with RunTestsInsideTransaction {
  ...
  test("return the correct results if an inner join is used") {
    val subquery = from(ordrs)((o) => groupBy(o.monthId).compute[Option[Int]](sum(o.qty)).orderBy(o.monthId))

    val mainquery = join(months, subquery)((m, sq) => select(m, sq.measures).on(m.id === sq.key))
  ...
  test("return the correct results if a left outer join is used") {
    ...
    val mainquery =
      join(months, subquery.leftOuter)((m, sq) => select(m, sq).on(m.id === sq.map(_.key)))
