...
abstract class MusicDbTestRun extends SchemaTester with QueryTester with RunTestsInsideTransaction with Matchers {
  ...
  test("OuterJoinWithSubQuery") {
    ...
    val q =
      join(artistsQ, songs.leftOuter)(
        (a, s) =>
          select((a, s))
            on (a.id === s.map(_.authorId))).toList

    val artistIdsWithoutSongs = q.filter(_._2 == None).map(_._1.id).toSet

    assertEquals(Set(alainCaron.id, hossamRamzy.id), artistIdsWithoutSongs, 'testOuterJoinWithSubQuery)
  ...
  lazy val songCountPerAlbumIdJoinedWithAlbum =
    from(songCountPerAlbumId(cds), cds)(
      (sc, cd) =>
        where(sc.key === cd.id)
  ...
  def songCountPerAlbumIdJoinedWithAlbumNested =
    from(songCountPerAlbumIdJoinedWithAlbumZ)(
      q =>
    ...
    from(
      ...
      (s, a) =>
        where(s.authorId === a.id or s.interpretId === a.id)
          select (a)
          orderBy (a.lastName desc)).distinct
...
//    testKeyedEntityImplicitLookup
  ...
  test("TimestampDownToMillis") {
    ...
    val dbAdapter = Session.currentSession.databaseAdapter
    if (!dbAdapter.isInstanceOf[MSSQLServer] && !dbAdapter
        .isInstanceOf[OracleAdapter]) { // FIXME or investigate millisecond handling of each DB:
  ...
  def inhibitedArtistsInQuery(inhibit: Boolean) =
    from(songs, artists.inhibitWhen(inhibit))(
      (s, a) =>
  ...
  test("DynamicQuery1") {
    ...
    val allSongs =
      from(songs)(
        s =>
          select(s)
            orderBy (s.title)).toList.map(s => s.id)
  ...
  def inhibitedSongsInQuery(inhibit: Boolean) =
    from(songs.inhibitWhen(inhibit), artists)(
      (s, a) =>
    ...
    from(artists)(
      a =>
        where(a.firstName between (s1, s2))
          select (a)
          orderBy (a.firstName asc)).map(a => a.firstName).toList
      ...
      from(songs)(
        s =>
          where(Option(s.genre) === genreFilter.?)
            select (s))
    assertEquals(listSongs(Some(Jazz)).size, songs.where(s => s.genre === Jazz).size, "expected all Jazz pieces")
...
//    println(cds.where(_.title in q2).statement)
  ...
  test("OptionalOuterJoin") {
     ...
     * least 2 CDs
     */
    val hossam = join(artists, cds.leftOuter)(
      (a, c) =>
     ...
     */
    val query1 = join(artists, cds.leftOuter.inhibitWhen(false))(
      (a, c) =>
        where(a.id === hossamRamzy.id)
          select ((a, c))
     ...
     */
    val query2 = join(artists, cds.leftOuter.inhibitWhen(true))(
      (a, c) =>
        where(a.id === ponchoSanchez.id)
          select ((a, c))
          on (c.map(_.mainArtist) === a.id))
