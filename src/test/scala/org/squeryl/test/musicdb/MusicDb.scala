...
import org.squeryl.test.musicdb.Genre.*
...
abstract class MusicDbTestRun extends SchemaTester with QueryTester with RunTestsInsideTransaction with Matchers {
  ...
  test("JoinWithCompute") {
    ...
    val q =
      join(artists, songs.leftOuter)((a, s) =>
        groupBy(a.id, a.firstName).compute(countDistinct(s.map(_.id))).on(a.id === s.map(_.authorId))
      )
  ...
  test("OuterJoinInOuter", SingleTestRun) {
    val firstSongs =
      from(songs)(s => groupBy(s.authorId).compute[Option[Int]](min(s.id)))

    val j2 =
      join(artists, firstSongs, cds.leftOuter)((a, fs, cd) =>
        select(a, cd).on(a.id === fs.measures, cd.map(_.mainArtist) === a.id)
      ): Query[(Person, Option[Cd])]

    val _ =
      join(artists, j2.leftOuter)((a, j2_) => select(a, j2).on(a.id === j2_.map(_._1.id))).toList
  }
  ...
  def songCountPerAlbum(cds: Queryable[Cd]) =
    from(cds, songs)((cd, song) => where(song.cdId === cd.id).groupBy(cd.title).compute(count).orderBy(cd.title))

  lazy val songCountPerAlbumFeaturingPoncho = songCountPerAlbum(
    from(songs, artists, cds)((s, a, cd) =>
      where(a.firstName === "Poncho" and s.interpretId === a.id and s.cdId === cd.id).select(cd)
    ).distinct
  )

  lazy val songsFeaturingPonchoNestedInFrom =
    from(songs, poncho)((s, a) => where(s.interpretId === a.id).select((s, a.firstName)).orderBy(s.title))

  def songCountPerAlbumId(cds: Queryable[Cd]) =
    from(cds, songs)((cd, song) => where(song.cdId === cd.id).groupBy(cd.id).compute(count))

  lazy val songCountPerAlbumIdJoinedWithAlbum =
    from(songCountPerAlbumId(cds), cds)((sc, cd) =>
      where(sc.key === cd.id).select((cd.title, sc.measures)).orderBy(cd.title)
    )

  lazy val songCountPerAlbumIdJoinedWithAlbumZ =
    from(songCountPerAlbumId(cds), cds)((sc, cd) => where(sc.key === cd.id).select((cd, sc)).orderBy(cd.title))

  def songCountPerAlbumIdJoinedWithAlbumNested =
    from(songCountPerAlbumIdJoinedWithAlbumZ)(q => select((q._1.title, q._2.measures)).orderBy(q._1.title))
  ...
  def avgSongCountForAllArtists =
    from(
      from(artists, songs)((a, s) => where(s.authorId === a.id).groupBy(a.id).compute(count))
    )((sonCountPerArtist) => compute[Option[Double]](avg(sonCountPerArtist.measures)))
...
//    testKeyedEntityImplicitLookup
    ...
    validateQuery(
      "songCountPerAlbumIdJoinedWithAlbumNested",
      q,
    ...
    validateQuery(
      "basicSelectUsingWhereOnQueryable",
      basicSelectUsingWhereOnQueryable,
      (a: Person) => a.id,
      List(mongoSantaMaria.id)
    )

    validateQuery(
      "basicSelectUsingWhereOnQueryableNested",
      basicSelectUsingWhereOnQueryableNested,
      (a: Person) => a.id,
      List(mongoSantaMaria.id)
    )
    ...
    validateQuery("songsFeaturingPoncho", songsFeaturingPoncho, (s: Song) => s.title, ponchoSongs)

    validateQuery(
      "songsFeaturingPonchoNestedInWhere",
      songsFeaturingPonchoNestedInWhere,
      (s: Song) => s.title,
      ponchoSongs
    )
    ...
    validateQuery(
      "songsFeaturingPonchoNestedInFrom",
      songsFeaturingPonchoNestedInFrom,
      (t: (Song, String)) => (t._1.id, t._2),
      List(
        (besameMama.id, ponchoSanchez.firstName),
    ...
    validateQuery(
      "artistsInvolvedInSongsm",
      artistsInvolvedInSongs(List(watermelonMan.id)),
      (a: Person) => a.id,
      List(ponchoSanchez.id, herbyHancock.id)
    )
  ...
  test("UpperAndLowerFuncs") {
    try {
      val q =
        from(artists)(a =>
          where(a.firstName.regex(".*on.*")).select(&(upper(a.firstName) || lower(a.firstName))).orderBy(a.firstName)
        )
      ...
      case e: UnsupportedOperationException =>
        println("testUpperAndLowerFuncs: regex not supported by database adapter")
    }
  }
  ...
  test("DynamicQuery1") {
    val testInstance = sharedTestInstance; import testInstance.*
    val allSongs =
      from(songs)(s => select(s).orderBy(s.title)).toList.map(s => s.id)
  ...
  test("DynamicQuery2") {
    ...
    validateQuery(
      "inhibitedSongsInQuery",
      songArtistsTuples,
      (t: (Option[Song], Person)) => (t._1.get.id, t._2.id),
      expected.toList
    )
  }
...
// TODO: REFACTOR Z (reintroduce case statements tests)
  ...
  test("AggregateQueryOnRightHandSideOfInOperator") {
    val testInstance = sharedTestInstance; import testInstance.*
    val q1 =
      from(cds)(cd => compute[Option[Int]](min(cd.id)))
...
//    println(cds.where(_.title in q2).statement)
  ...
  test("AggregateComputeInSubQuery") {
    ...
    val q2 =
      from(cds, q1)((cd, q) => where(cd.id === q.measures.get).select(cd))
    ...
    val q3 =
      from(cds)(cd => groupBy(cd.id))

    val q4 =
      from(cds, q3)((cd, q) => where(cd.id === q.key).select(cd))
  ...
  test("OptionalOuterJoin") {
    ...
    val hossam = join(artists, cds.leftOuter)((a, c) =>
      where(a.id === hossamRamzy.id).select((a, c)).on(a.id === c.map(_.mainArtist))
    ).toList
    hossam.size should be(1)
    hossam.head._2 should be(None)
    join(artists, cds)((a, c) =>
      where(a.id === ponchoSanchez.id).select((a, c)).on(a.id === c.mainArtist)
    ...
    val query1 = join(artists, cds.leftOuter.inhibitWhen(false))((a, c) =>
      where(a.id === hossamRamzy.id).select((a, c)).on(c.map(_.mainArtist) === a.id)
    )
    ...
    val query2 = join(artists, cds.leftOuter.inhibitWhen(true))((a, c) =>
      where(a.id === ponchoSanchez.id).select((a, c)).on(c.map(_.mainArtist) === a.id)
    )
    query2.toList.size should be(1)
  }

  test("Inhibit single LogicalBoolean") {
    from(artists)(a => where((a.age === 1000000).inhibitWhen(true)).select(a)).toList.size should be > (0)
  ...
  test("Inhibit one side of LogicalBoolean") {
    ...
    // Left inhibit
    from(artists)(a =>
      where((a.age === 1000000).inhibitWhen(true) and a.id === hossamRamzy.id).select(a)
    ).toList.size should be(1)
    // Right inhibit
    from(artists)(a =>
      where(a.id === hossamRamzy.id and (a.age === 1000000).inhibitWhen(true)).select(a)
    ).toList.size should be(1)
  }

  test("Inhibit both sides of LogicalBoolean") {
    from(artists)(a =>
      where((a.age === 1000000).inhibitWhen(true) and (a.id between (999, 1000)).inhibitWhen(true)).select(a)
