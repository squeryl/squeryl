...
class SchoolDb2 extends Schema {
  ...
  val entryToComments = oneToManyRelation(entries, comments).via((e, c) => e.id === c.entryId)
  ...
  val courseAssignments =
    manyToManyRelation(professors, courses, "CourseAssignmentZ").via[CourseAssignment]((p, c, a) =>
...
//  }
  ...
  test("select using query value") {
    ...
    val q: Query[String] =
      from(subjects)(s =>
        where(s.name === "Philosophy").select(&(from(subjects)(s2 => where(s2.name === s.name).select(s2.name))))
      )
  ...
  test("CompositeEquality") {
    ...
    val qA2 =
      from(courseAssignments)(ca => where(ca.id === (a.courseId, a.professorId)).select(ca))
  ...
  test("UniquenessConstraint") {
    ...
    if (!exceptionThrown)
      org.squeryl.internals.Utils
        .throwError("testUniquenessConstraint" + " failed, unique constraint violation occurred")
  ...
  test("Inequality with query on right hand side", SingleTestRun) {
    ...
    val belowOrEqualToAvg =
      from(courseSubscriptions)(p =>
        where(p.grade lte from(courseSubscriptions)(p => compute[Option[Float]](avg(p.grade)))).select(p)
      ).toList
