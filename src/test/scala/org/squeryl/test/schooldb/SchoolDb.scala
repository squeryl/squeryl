...
class CourseSubscription(var courseId: Int, var studentId: Int) extends SchoolDbObject {

  override def toString = "CourseSubscription:" + id
}

class CourseAssignment(var courseId: Int, var professorId: Long) extends SchoolDbObject {
...
class TestInstance(schema: SchoolDb) {
  ...
  val xiao =
    students.insert(new Student("Xiao", "Jimbao Gallois", Some(24), 2, Some(oneHutchissonStreet.id), Some(true)))
  val georgi =
    students.insert(new Student("Georgi", "Balanchivadze Fourrier", Some(52), 1, Some(oneHutchissonStreet.id), None))
  val pratap =
    students.insert(new Student("Pratap", "Jamsetji Bach", Some(25), 1, Some(oneTwoThreePieIXStreet.id), None))
  val gontran =
    students.insert(new Student("Gontran", "Plourde", Some(25), 1, Some(oneHutchissonStreet.id), Some(true)))
...
abstract class FullOuterJoinTests extends SchoolDbTestBase {
  ...
  test("NewLeftOuterJoin1Reverse") {
    ...
    val leftOuterJoinStudentAddresses =
      join(addresses.leftOuter, students)((a, s) => select((s, a)).orderBy(s.id).on(s.addressId === a.map(_.id)))
          ...
          .where(
            exists(
              join(qStudents, qStudents)((s2, s3) =>
                where(s2.name === "Xiao" and exists(from(qStudents)(s4 => where(s4.name === "Xiao").select(s4))))
                  .select(s2)
                  .on(s2.name === s3.name)
              )
            ) and s.name === "Xiao"
...
abstract class SchoolDbTestRun extends SchoolDbTestBase {
  ...
  def avgStudentAge() =
    from(students)(s => compute[Option[Float]](avg(s.age)))
  ...
  def addressesOfStudentsOlderThan24 =
    from(students, addresses)((s, a) =>
      where((24 lt s.age) and (24 lt s.age)).select(&(a.numberz || " " || a.streetName || " " || a.appNumber))
    )
...
//  }
  ...
  test("update to null") {
    ...
    update(students)(p => where(p.id === rejan.id).set(p.isMultilingual := None))
  }
  ...
  class RawQuery(query: String, args: collection.Seq[Any]) {
    ...
    def toTuple[A1, A2]()(implicit f1: TypedExpressionFactory[A1, _], f2: TypedExpressionFactory[A2, _]) = {
      ...
      (m1.convertFromJdbc(m1.extractNativeJdbcValue(rs, 1)), m2.convertFromJdbc(m2.extractNativeJdbcValue(rs, 2)))
    }
  }
  ...
  test("testServerSideFunctionCall") {
    ...
    val s =
      from(students)(s => where(lower(s.name) === lower("GONtran")).select((&(lower(s.name)), &(upper("zaza"))))).single
  ...
  test("SingleOption") {
    ...
    val q2 =
      from(students)(s => where(s.name like "Gontran").select(s.id).orderBy(s.name))
...
//    val mandarin = courses.insert(new Course("Mandarin 101", feb2010, None, 0, None, true))
    ...
    assert(expected == result, "testDateComparisonInWhereClause" + " expected " + expected + " got " + result)
  }
...
//    val mandarin = courses.insert(new Course("Mandarin 101", feb2010, None, 0, None, true))
    ...
    assert(expected == result1, "testDateOptionComparisonInWhereClause" + " expected " + expected + " got " + result1)

    assert(Nil == result2, "testDateOptionComparisonInWhereClause" + " expected " + expected + " got " + result2)

    assert(expected == result3, "testDateOptionComparisonInWhereClause" + " expected " + expected + " got " + result3)
  }
...
//    val mandarin = courses.insert(new Course("Mandarin 101", feb2010, None, 0, None, true))
    ...
    assert(expected == result, "testNVLFunction" + " expected " + expected + " got " + result)
  }
...
//    courses.update(ht)
    ...
    update(courses)(c => where(c.id === heatTransfer.id).set(c.confirmed := true))
...
//    courses.update(ht)
    ...
    update(courses)(c => where(c.id === heatTransfer.id).set(c.confirmed := false))
  ...
  test("PartialUpdate1") {
    ...
    val q =
      from(courses)(c => select((c.id, c.meaninglessLong, c.meaninglessLongOption)).orderBy(c.id))

    val b4 = q.toList

    var nRows = courses.update(c =>
      where(c.id gt -1)
        .set(c.meaninglessLong := 123L, c.meaninglessLongOption := c.meaninglessLongOption.+(456L)(optionLongTEF))
    ...
    nRows = update(courses)(c =>
      where(c.id gt -1)
        .set(c.meaninglessLong := 0L, c.meaninglessLongOption := c.meaninglessLongOption.-(456L)(optionLongTEF))
    )
  ...
  test("HavingClause") {
    ...
    val q =
      from(professors)(p => groupBy(p.id, p.yearlySalary).having(p.yearlySalary gt 75.0f))
  ...
  test("PartialUpdateWithSubQueryInSetClause") {
    ...
    val expected: Float = from(professors)(p0 =>
      where(tournesol.id === p0.id or p0.id === zarnitsyn.id)
        .compute(nvl(avg(p0.yearlySalary)(optionFloatTEF), 123)(optionFloatTEF))
    )

    update(professors)(p =>
      where(p.id === tournesol.id).set(
        p.yearlySalary := from(professors)(p0 =>
          where(p.id === p0.id or p0.id === zarnitsyn.id)
            .compute(nvl(avg(p0.yearlySalary)(optionFloatTEF), 123)(optionFloatTEF))
        )
      )
    )
    ...
    update(professors)(p => where(p.id === tournesol.id).set(p.yearlySalary := 80.0f))
  ...
  test("OptimisticCC1") {
    ...
    ex.getOrElse(
      org.squeryl.internals.Utils.throwError("StaleUpdateException should have get thrown on concurrent update test.")
    )
  ...
  test("BigDecimal") {
    ...
    val babaZula =
      professors.insert(new Professor("Baba Zula", 80.0f, Some(70.5f), 80.0f, Some(260.1234567f: BigDecimal)))
    ...
    update(professors)(p =>
      where(p.id === babaZula.id).set(
        p.yearlySalaryBD := p.yearlySalaryBD
          .plus(10)(bigDecimalTEF)
          .minus(5)(bigDecimalTEF)
          .times(4)(bigDecimalTEF)
          .div(2)(bigDecimalTEF, bigDecimalTEF)
  ...
  test("InWithCompute") {
    val z0 =
      from(students)(s2 => where(s2.age gt 0).compute[Option[Int]](min(s2.age)))

    val q2 = (z0: Query[Measures[Option[Int]]]): Query[Option[Int]]

    val q3 =
      from(students)(s => where(s.age.isNotNull and s.age.in(q2)).select(s))
  ...
  test("NewLeftOuterJoin1") {
    ...
    val leftOuterJoinStudentAddresses =
      join(students, addresses.leftOuter)((s, a) => select((s, a)).orderBy(s.id).on(s.addressId === a.map(_.id)))
  ...
  test("NewLeftOuterJoin2") {
    ...
    val leftOuterJoinStudentAddresses =
      join(students, addresses.leftOuter, addresses.leftOuter)((s, a, a2) =>
        select((s, a, a2)).orderBy(s.id).on(s.addressId === a.map(_.id), s.addressId === a2.map(_.id))
      )
  ...
  test("NewLeftOuterJoin3") {
    ...
    val leftOuterJoinStudentAddressesAndCourseSubs =
      join(students, addresses.leftOuter, courseSubscriptions)((s, a, cs) =>
        select((s, a, cs)).orderBy(s.id, cs.courseId).on(s.addressId === a.map(_.id), s.id === cs.studentId)
      )
  ...
  test("TestYieldInspectionLeakViaCGLIB") {
    ...
    join(group, others)((g, o) => select(g.measures.get, o).on(g.key === o.testId)).toList
  }

  test("Exists") {
    val studentsWithAnAddress =
      from(students)(s => where(exists(from(addresses)((a) => where(s.addressId === a.id).select(a.id)))).select(s))
  ...
  test("NotExists") {
    val studentsWithNoAddress =
      from(students)(s => where(notExists(from(addresses)((a) => where(s.addressId === a.id).select(a.id)))).select(s))
  ...
  test("VeryNestedExists") {
    ...
    val studentsWithAnAddress =
      from(qStudentsFromStudents)(s =>
        where(exists(from(addresses)((a) => where(s.addressId === a.id).select(a.id)))).select(s)
      )
        ...
        where(
          exists(
            from(addresses)((a) =>
              where(
                s.addressId in
                  (from(addresses)((a2) => where(a2.id === a.id and s.addressId === a2.id).select(a2.id)))
              ).select(a.id)
            )
          )
        ).select(s)
  ...
  test("selectFromExists") {
    val qStudents = from(students)((s) => select(s))
    val studentsWithAnAddress =
      from(qStudents)(s => where(exists(from(addresses)((a) => where(s.addressId === a.id).select(a)))).select(s))
    val qAStudentIfHeHasAnAddress =
      from(studentsWithAnAddress)(s => where(s.name === "Xiao").select(s))
...
""")
        ...
        val seqName = (new OracleAdapter)
          .createSequenceName(Issue14Schema.professors.posoMetaData.findFieldMetaDataForProperty("id").get)
