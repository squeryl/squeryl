...
trait GeneratedKeyDef[A,K] {
  sef: KeyedEntityDef[A,K] =>
  
  /**
   * If the id is autogenerated, this method allows the framework "set it".
   * Note that this does not necessarily mean that the entity must be mutable.
   * The value that is returned from this meothd can be an immutable copy.
   */
  def setId(a: A, id: K): A
