...
class PosoMetaData[T](val clasz: Class[T], val schema: Schema, val viewOrTable: View[T]) {
  ...
  val constructor =
    _const.headOption
      .orElse(
        org.squeryl.internals.Utils.throwError(clasz.getName +
  ...
  val (_fieldsMetaData, primaryKey): (Iterable[FieldMetaData], Option[Either[FieldMetaData, Method]]) = {
    ...
    for (e <- name2MembersMap) {
      ...
      if (isImplicitMode && _groupOfMembersIsProperty(property)) {
        ...
        try {
          val r =
            FieldMetaData.factory.build(this, name, property, sampleInstance4OptionTypeDeduction, isOptimisitcCounter)
          fmds.append(r)
        } catch {
          case e: Exception =>
...
//      println("CT: " + ct.getParameterTypes.map(c=>c.getName).mkString(","))
    ...
    r.sortWith(
      (a: (Constructor[_], Array[Object]), b: (Constructor[_], Array[Object])) => a._2.length < b._2.length
    )
  }
  ...
  private def _groupOfMembersIsProperty(
    property: (Option[Field], Option[Method], Option[Method], Set[Annotation])): Boolean = {
  ...
  private def _fillWithMembers(clasz: Class[_], members: ArrayBuffer[(Member, HashSet[Annotation])]): Unit = {
    ...
    for (m <- clasz.getMethods
      if (m.getDeclaringClass != classOf[Object]) && _includeFieldOrMethodType(m.getReturnType)) {
