...
class PosoMetaData[T](val clasz: Class[T], val schema: Schema, val viewOrTable: View[T]) {
  ...
  val constructor =
    _const.headOption.orElse(sys.error(clasz.getName +
...
//      println("CT: " + ct.getParameterTypes.map(c=>c.getName).mkString(","))
  ...
  def _tryToCreateParamArray(
    ...
    if(params.length >= 1) {
      ...
      if(cn == test)
        sys.error("inner classes are not supported, except when outer class is a singleton (object)\ninner class is : " + cn)
    }
  ...
  private def _noOptionalColumnDeclared =
    sys.error("class " + clasz.getName + " has an Option[] member with no Column annotation with optionType declared.")
