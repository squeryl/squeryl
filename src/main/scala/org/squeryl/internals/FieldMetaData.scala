...
class FieldMetaData(
  ...
  def sequenceName: String = {
    ...
    val ai = _columnAttributes.collectFirst { case a: AutoIncremented => a }.getOrElse(org.squeryl.internals.Utils
      .throwError(this + " is not declared as autoIncremented, hence it has no sequenceName"))
  ...
  if (isIdFieldOfKeyedEntity && !classOf[CompositeKey].isAssignableFrom(wrappedFieldType)) {
    schema
      ...
      .foreach(ca => {
        ...
        if (ca.isInstanceOf[AutoIncremented] && !(wrappedFieldType
            .isAssignableFrom(classOf[java.lang.Long]) || wrappedFieldType.isAssignableFrom(
...
object FieldMetaData {
  ...
  private def _createCustomTypeFactory(
    ...
    ownerClass: Class[_],
    typeOfField: Class[_]): Option[AnyRef => Product1[Any] with AnyRef] = {
  ...
  def defaultFieldLength(fieldType: Class[_], fmd: FieldMetaData) = {
    ...
    else if (classOf[java.math.BigDecimal].isAssignableFrom(fieldType) || classOf[scala.math.BigDecimal]
  ...
  def createDefaultValue(
    ...
    p: Class[_],
    t: Option[Type],
    optionFieldsInfo: Option[Column]): Object = {
