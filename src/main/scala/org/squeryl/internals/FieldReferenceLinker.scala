...
object FieldReferenceLinker {
  ...
  private def _takeLastAccessedUntypedFieldReference: SelectElementReference[_] =
    FieldReferenceLinker.takeLastAccessedFieldReference match {
      ...
      case None => sys.error("Thread local does not have a last accessed field... this is a severe bug !")
  }
  ...
  def determineColumnsUtilizedInYeldInvocation(q: QueryExpressionNode[_], rsm: ResultSetMapper, selectClosure: ()=>AnyRef) = {
    ...
    try {
      ...
      if(res0 == null)
        sys.error("query " + q + " yielded null")
...
//      sys.error("A query that returns a AnyVal cannot be nested " + Utils.failSafeString(FieldReferenceLinker.inspectedQueryExpressionNode.toString))
//    else
