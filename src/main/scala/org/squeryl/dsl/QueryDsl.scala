...
//      queryToIterable(view2QueryAll(t))
  ...
  class ManyToManyRelationBuilder[L, R](
    ...
    def via[A](f: (L,R,A)=>Tuple2[EqualityExpression,EqualityExpression])(implicit manifestA: ClassTag[A], schema: Schema, kedA: KeyedEntityDef[A,_]) = {
  ...
  class ManyToManyRelationImpl[L, R, A](
    ...
    extends Table[A](nameOverride.getOrElse(schema.tableNameFromClass(aClass)), aClass, schema, None, Some(kedA), None) with ManyToManyRelation[L,R,A] {
    thisTableOfA =>
