...
trait TypedExpression[A1,T1] extends ExpressionNode {
  ...
  def :=[B] (b: B)(implicit evB: B => TypedExpression[A1, T1]): UpdateAssignment =
    new UpdateAssignment(_fieldMetaData, evB(b) : TypedExpression[A1,T1])

  def :=(q: Query[Measures[A1]]) =
    new UpdateAssignment(_fieldMetaData, q.ast)

  def defaultsTo[B](b: B)(implicit evB: B => TypedExpression[A1, T1]): DefaultValueAssignment =
    new DefaultValueAssignment(_fieldMetaData, evB(b) : TypedExpression[A1,T1])
