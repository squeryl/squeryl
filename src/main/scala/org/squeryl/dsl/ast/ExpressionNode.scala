...
trait ExpressionNode {
  ...
  def filterDescendantsOfType[T](implicit manifest: ClassTag[T]) =
    _filterDescendants(
      this,
...
class CompositeKeyAttributeAssignment(val group: CompositeKey, _columnAttributes: collection.Seq[ColumnAttribute])
  ...
  override def isIdFieldOfKeyedEntity = {
    ...
    fmdHead.parentMetaData.viewOrTable.ked.map(k => Some(k.idPropertyName) == group._propertyName).getOrElse(false)
  }
