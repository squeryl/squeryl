...
class ExportedSelectElement
  ...
  var outerScopes:List[ExpressionNode] = Nil
  ...
  def possibleOuterScopeRef:Boolean = innerTarget == None

  private def isDirectOuterReference: Boolean = outerScopes.exists((outer) => outer.useSite == selectElement.parentQueryable)

  private def outerTarget: Option[SelectElement] = {
    val q =
      for (outer <- outerScopes;
           subQuery <- outer.useSite.asInstanceOf[QueryExpressionElements].subQueries;
